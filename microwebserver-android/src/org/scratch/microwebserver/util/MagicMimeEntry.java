...
class MagicMimeEntry
{
	...
	private List<MagicMimeEntry> subEntries=new ArrayList<MagicMimeEntry>();
	...
	public MagicMimeEntry(List<String> entries)
	{
		this(0,null,entries);
	}
	...
	private void initContentNumber()
	{
		...
		switch(type)
		{
			...
			case BELONG_TYPE:
			{
				...
				{
					contentNumber=Long
							.parseLong(content.substring(2).trim(),16); // Without
																		// the
																		// trim(),
																		// I
																		// got
	...
	private ByteBuffer readBuffer(byte[] content) throws IOException
	{
		...
		try
		{
			switch(getType())
			{
				case MagicMimeEntry.STRING_TYPE:
				{
					...
					// documentation.
					// This is an extension to the magic rules and is provided
					// by
					// this utility.
					// It allows for better matching of some text based files
					// such
					// as XML files
		...
		{
			// Content passed in is to small for the comparison so just ignore
			// this match
			return null;
		}
		return buf;
	...
	private int getInputStreamMarkLength()
	{
		...
		for(Iterator<MagicMimeEntry> it=subEntries.iterator();it.hasNext();)
		{
	...
	private static String stringWithEscapeSubstitutions(String s)
	{
		...
		while(indx<len)
		{
			...
			if(c=='\\')
			{
				...
				{ // Bugfix by Marco: Implemented reading
	...
	public boolean containsMimeType(String mimeType)
	{
		...
		for(Iterator<MagicMimeEntry> it=subEntries.iterator();it.hasNext();)
		{
			MagicMimeEntry subEntry=it.next();
