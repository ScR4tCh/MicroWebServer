...
public abstract class DomUtils
{
	...
	{
		List<Node> nodeList=DomUtils.copyNodeList(source.getChildNodes());
	 ...
	 * 
	 * @param newNode
	 *            New node - added in same location as oldNode.
	 * @param oldNode
	 *            Old node - removed.
	 */
	public static void replaceNode(Node newNode,Node oldNode)
	{
		...
		if(parentNode==null)
		{
			System.out.println("Cannot replace node ["+oldNode+"] with ["
					+newNode+"]. ["+oldNode+"] has no parent.");
		}
		else
		{
	 ...
	 * 
	 * @param newNodes
	 *            New nodes - added in same location as oldNode.
	 * @param oldNode
	 *            Old node - removed.
	 */
	...
	public static void replaceNode(NodeList newNodes,Node oldNode,boolean clone)
	{
		...
		if(parentNode==null)
		{
			System.out.println("Cannot replace ["+oldNode
					+"] with a NodeList. ["+oldNode+"] has no parent.");
			return;
		}

		int nodeCount=newNodes.getLength();
		List<Node> nodeList=DomUtils.copyNodeList(newNodes);
		...
		if(parentNode instanceof Document)
		{
			List<Element> elements=DomUtils.getElements(newNodes,"*",null);

			if(!elements.isEmpty())
			{
				System.out
						.println("Request to replace the Document root node with a 1+ in length NodeList.  Replacing root node with the first element node from the NodeList.");
				parentNode.removeChild(oldNode);
			...
			{
				System.out
						.println("Cannot replace document root element with a NodeList that doesn't contain an element node.");
			}
		}
		else
		{
			for(int i=0;i<nodeCount;i++)
			{
				if(clone)
				{
					parentNode.insertBefore((nodeList.get(i)).cloneNode(true),
							oldNode);
				}
				else
				{
	 ...
	 *            Node to be inserted.
	 * @param refNode
	 *            Reference node before which the supplied nodes should be
	 *            inserted.
	 */
	...
	public static void insertBefore(NodeList newNodes,Node refNode)
	{
		...
		if(parentNode==null)
		{
			System.out.println("Cannot insert a NodeList before ["+refNode
					+"]. ["+refNode+"] has no parent.");
			return;
		}
		...
		if(parentNode instanceof Document)
		{
			...
			{
				System.out
						.println("Request to insert a NodeList before the Document root node.  Will replace the root element with the 1st element node from the NodeList.");
				parentNode.removeChild(refNode);
				parentNode.appendChild((Node)elements.get(0));
			}
			else
			{
				System.out
						.println("Cannot insert beforen the document root element from a NodeList that doesn't contain an element node.");
			}
	 ...
	 *            The tag name of the replacement element.
	 * @param keepChildContent
	 *            <code>true</code> if the target element's child content is to
	 *            be copied to the replacement element, false if not. Default
	 *            <code>true</code>.
	 * @param keepAttributes
	 *            <code>true</code> if the target element's attributes are to be
	 *            copied to the replacement element, false if not. Default
	 *            <code>true</code>.
	 * @return The renamed element.
	 */
	public static Element renameElement(Element element,
			String replacementElement,boolean keepChildContent,
			boolean keepAttributes)
	{

		Element replacement=element.getOwnerDocument().createElement(
				replacementElement);
		if(keepChildContent)
		{
		...
		if(keepAttributes)
		{
			...
			for(int i=0;i<attributeCount;i++)
			{
				...
				replacement.setAttribute(attribute.getName(),
						attribute.getValue());
			}
		}
	 ...
	 * <p/>
	 * Tries to manage scenarios where a request is made to remove the root
	 * element. Cannot remove the root element in any of the following
	 * situations:
	 * <ul>
	...
	public static void removeElement(Element element,boolean keepChildren)
	{
		...
		if(parent==null)
		{
			System.out.println("Cannot remove element ["+element+"]. ["+element
					+"] has no parent.");
			return;
		}
		...
		if(parent instanceof Document)
		{
			...
			if(!keepChildren)
			{
				System.out.println("Cannot remove document root element ["
						+DomUtils.getName(element)
			...
			else
			{
				...
				{
					System.out
							.println("Cannot remove empty document root element ["
									+DomUtils.getName(element)+"].");
				}
			}
		}
	 ...
	 * the list will remain the same length. Using the NodeList can result in
	 * problems because elements can get removed from the list while we're
	 * iterating over it.
	 * 
	 * @param nodeList
	 *            Nodelist to copy.
	 ...
	 * @param attribName
	 *            The attribute name.
	 * @return True if the attribute value is "true" (case insensitive),
	 *         otherwise false.
	 */
	 ...
	 *            The element.
	 * @param namespaceURI
	 *            Namespace URI of the required attribute.
	 * @param attribName
	 *            The attribute name.
	 * @return True if the attribute value is "true" (case insensitive),
	 *         otherwise false.
	...
	/**
	 * Get the parent element of the supplied element having the specified tag
	 * name.
	 * 
	 * @param child
	 *            Child element.
	 ...
	 * @return The first parent element of "child" having the tagname
	 *         "parentName", or null if no such parent element exists.
	 */
	 ...
	 *            Parent element local name.
	 * @param namespaceURI
	 *            Namespace URI of the required parent element, or null if a
	 *            non-namespaced get is to be performed.
	 * @return The first parent element of "child" having the tagname
	 *         "parentName", or null if no such parent element exists.
	 */
	public static Element getParentElement(Element child,
			String parentLocalName,String namespaceURI)
	{
	 ...
	 * <p/>
	 * Returns the {@link Node#getLocalName() localName} of the element if set
	 * (namespaced element), otherwise the element's
	 * {@link Element#getTagName() tagName} is returned.
	 * 
	 * @param element
	 *            The element.
	 ...
	 *            The attribute to get.
	 * @param namespaceURI
	 *            Namespace URI of the required attribute, or null to perform a
	 *            non-namespaced get.
	 * @return The attribute value, or <code>null</code> if unset.
	 */
	public static String getAttributeValue(Element element,
			String attributeName,String namespaceURI)
	{
	...
	public static Node getPreviousSibling(Node node,short nodeType)
	{
		...
		if(parent==null)
		{
			System.out.println("Cannot get node ["+node+"] previous sibling. ["
					+node+"] has no parent.");
			return null;
		}
	 ...
	 * 
	 * @param node
	 *            Node whose siblings are to be counted.
	 * @param nodeType
	 *            The DOM {@link Node} type of the siblings to be counted.
	 * @return The number of siblings of the supplied type before the supplied
	 *         node.
	 */
	public static int countNodesBefore(Node node,short nodeType)
	{
		...
		if(parent==null)
		{
			System.out.println("Cannot count nodes before ["+node+"]. ["+node
					+"] has no parent.");
			return 0;
		}
	 ...
	 *             if the supplied {@link Node Nodes} don't have the same parent
	 *             node i.e. are not sibling nodes.
	 */
	public static int countNodesBetween(Node node1,Node node2,short nodeType)
	{
		...
		if(parent1==null)
		{
			System.out.println("Cannot count nodes between ["+node1+"] and ["
					+node2+"]. ["+node1+"] has no parent.");
			return 0;
		}
		...
		{
			System.out.println("Cannot count nodes between ["+node1+"] and ["
					+node2+"]. These nodes do not share the same sparent.");
			return 0;
		}
	...
	/**
	 * Count the DOM nodes before the supplied node, not including the node
	 * itself.
	 * <p/>
	 * Counts the sibling nodes.
	 * 
	...
	/**
	 * Count the DOM nodes between the supplied sibling nodes, not including the
	 * nodes themselves.
	 * <p/>
	 * Counts the sibling nodes.
	 * 
	...
	/**
	 * Count the DOM element nodes before the supplied node, having the
	 * specified tag name, not including the node itself.
	 * <p/>
	 * Counts the sibling nodes.
	 * 
	 * @param node
	 *            Node whose element siblings are to be counted.
	 * @param tagName
	...
	public static String getTextBefore(Node node)
	{
		...
		if(parent==null)
		{
			System.out.println("Cannot get text before node ["+node+"]. ["+node
					+"] has no parent.");
			return "";
		}
	...
	public static String getTextBetween(Node node1,Node node2)
	{
		...
		if(parent1==null)
		{
			System.out.println("Cannot get text between nodes ["+node1
	...
	public static String getXPath(Node node)
	{
		...
		switch(node.getNodeType())
		{
			...
			case Node.COMMENT_NODE:
				int commentNum=DomUtils
						.countNodesBefore(node,Node.COMMENT_NODE);
				xpath.append("/{COMMENT}["+commentNum+1+"]");
				break;
			case Node.CDATA_SECTION_NODE:
				int cdataNum=DomUtils.countNodesBefore(node,
	...
	/**
	 * Get the child element having the supplied localname, position and
	 * namespace.
	 * <p/>
	 * Can be used instead of XPath.
	 * 
	 * @param parent
	 *            Parent element to be searched.
	 * @param localname
	 *            Localname of the element required.
	 * @param position
	 *            The position of the element relative to other sibling elements
	 *            having the same name (and namespace if specified) e.g. if
	 *            searching for the 2nd &ltinput&gt; element, this param needs
	 ...
	 *            to have a value of 2.
	 * @param namespaceURI
	 *            Namespace URI of the required element, or null if a namespace
	 *            comparison is not to be performed.
	 * @return The element at the requested position, or null if no such child
	 *         element exists on the parent element.
	 */
	public static Element getElement(Element parent,String localname,
			int position,String namespaceURI)
	{
	 ...
	 *            Parent element to be searched.
	 * @param localname
	 *            Localname of the element required. Supports "*" wildcards.
	 * @param namespaceURI
	 *            Namespace URI of the required element, or null if a namespace
	 *            comparison is not to be performed.
	 ...
	 * 
	 * @param nodeList
	 *            List of DOM nodes on which to perform the search.
	 * @param localname
	 *            Localname of the element required. Supports "*" wildcards.
	 * @param namespaceURI
	 *            Namespace URI of the required element, or null if a namespace
	 ...
	 *         child elements exist on the parent element.
	 */
	public static List<Element> getElements(NodeList nodeList,String localname,
			String namespaceURI)
	{
