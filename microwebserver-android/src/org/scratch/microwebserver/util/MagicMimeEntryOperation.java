...
/**
 * Quote from <a
 * href="http://linux.die.net/man/5/magic">http://linux.die.net/man/5/magic</a>:
 * <p>
 * Numeric values may be preceded by a character indicating the operation to be
 * performed. It may be =, to specify that the value from the file must equal
 * the specified value, &lt;, to specify that the value from the file must be
 * less than the specified value, &gt;, to specify that the value from the file
 * must be greater than the specified value, &amp;, to specify that the value
 * from the file must have set all of the bits that are set in the specified
 * value, ^, to specify that the value from the file must have clear any of the
 * bits that are set in the specified value, or ~, the value specified after is
 * negated before tested. x, to specify that any value will match. If the
 * character is omitted, it is assumed to be =. For all tests except string and
 * regex, operation ! specifies that the line matches if the test does not
 * succeed.
 * </p>
 * <p>
 ...
 * magic file. This means that a line can match any string, and then presumably
 * print that string, by doing &gt;\0 (because all strings are greater than the
 * null string).
 * </p>
 * 
...
{
	private static final Map<Character,MagicMimeEntryOperation> operationID2operation=new HashMap<Character,MagicMimeEntryOperation>();
	...
	public static MagicMimeEntryOperation getOperationForStringField(String content)
	{
		...
		// String and regex do only support a subset of the operations =>
		// filter.
		if(EQUALS.equals(operation)||LESS_THAN.equals(operation)
				||GREATER_THAN.equals(operation))
			return operation;
		else
			return EQUALS;
	}
